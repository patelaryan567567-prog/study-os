...
import { Search, Bell, Maximize2, X, CheckCircle, Clock, AlertTriangle } from 'lucide-react';
...
const SAMPLE_NOTIFICATIONS = [
  { id: '1', type: 'success' as const, title: 'Study streak active!', body: 'You have studied 3 days in a row.', time: '2m ago' },
  { id: '2', type: 'warning' as const, title: 'Backlog reminder', body: 'You have 4 overdue tasks.', time: '1h ago' },
  { id: '3', type: 'info' as const, title: 'Weekly goal', body: 'You are 60% toward your 12h weekly goal.', time: '3h ago' },
];

const notifIcon = {
  success: <CheckCircle size={14} style={{ color: 'var(--color-success)' }} />,
  warning: <AlertTriangle size={14} style={{ color: 'var(--color-warning)' }} />,
    ...
    >
      ...
      <div className="flex items-center gap-1">
        ...
        <div ref={searchRef} style={{ position: 'relative' }}>
          <Button variant="ghost" size="icon" title="Search (Ctrl+K)" onClick={() => { setSearchOpen(o => !o); setNotifOpen(false); }}>
            <Search size={15} />
          </Button>
        ...
        <div ref={notifRef} style={{ position: 'relative' }}>
          <Button variant="ghost" size="icon" title="Notifications" onClick={() => { setNotifOpen(o => !o); setSearchOpen(false); }}
            className="relative">
            <Bell size={15} />
              ...
              <div style={{ maxHeight: 320, overflowY: 'auto' }}>
                {notifications.length === 0 ? (
                  <div style={{ padding: '40px 16px', textAlign: 'center' }}>
                    <Bell size={28} style={{ color: 'var(--color-text-muted)', margin: '0 auto 10px' }} />
                    <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--color-text-secondary)' }}>No notifications</p>
                    <p style={{ fontSize: 12, color: 'var(--color-text-muted)', marginTop: 4 }}>You're all caught up!</p>
                  </div>
                ) : notifications.map(n => (
                  <div key={n.id} style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <p style={{ fontSize: 12, color: 'var(--color-text-secondary)', marginTop: 2 }}>{n.body}</p>
                      <p style={{ fontSize: 11, color: 'var(--color-text-muted)', marginTop: 4 }}>{n.time}</p>
                    </div>
                    <button onClick={() => setNotifications(p => p.filter(x => x.id !== n.id))}
                      style={{ color: 'var(--color-text-muted)', background: 'none', border: 'none', cursor: 'pointer', flexShrink: 0, display: 'flex' }}>
                      <X size={12} />
                    </button>
                  </div>
                ))}
        ...
        {/* Fullscreen */}
        <Button variant="ghost" size="icon" title="Fullscreen" onClick={toggleFullscreen}>
          <Maximize2 size={15} />
        </Button>
