...
function getErrorMessage(error: unknown): string {
  ...
  // Native (Android) Google sign-in failures surface as plain messages —
  // translate them so the user knows exactly what to fix.
  if (isNative()) {
    const msg = error.message || "";
    if (
      /google-services/i.test(msg) ||
      /not initialized/i.test(msg) ||
      /FirebaseApp/i.test(msg) ||
      /configuration/i.test(msg)
    ) {
      return "Google sign-in needs the Android app registered in Firebase. Add the Android app (package com.studyos.app) in the Firebase console, put google-services.json in android/app/, then rebuild the APK.";
    }
    if (/play services/i.test(msg) || /play-services/i.test(msg)) {
      return "Google Play services is missing or outdated on this device. Please update Google Play services and try again.";
    }
    if (/not implemented/i.test(msg) || /Unavailable/i.test(msg)) {
      return "This build doesn't include the native Google sign-in plugin. Run `npx cap sync android` and rebuild the APK.";
    }
    if (/developer[_ ]?error|status[_ ]?code:?\s*10|api exception: 10/i.test(msg)) {
      return "This APK is not registered for Google sign-in. Add this build's SHA-1 certificate for package com.studyos.app in Firebase, download the updated google-services.json, and rebuild the APK.";
    }
  ...
  const messages: Record<string, string> = {
    ...
    "auth/argument-error":
      "Firebase Auth configuration is invalid. Check Google/Email sign-in is enabled and the authorized domains include localhost.",
  ...
  return messages[code] ?? messages[error.message] ?? error.message;
}
