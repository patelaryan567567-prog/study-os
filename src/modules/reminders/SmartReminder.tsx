...
export function SmartReminder() {
  ...
  useEffect(() => {
    persistStore(STORAGE_KEY, reminders);
  }, [reminders]);

  // The login/periodic sync writes restored data straight to localStorage —
  // reload when it lands so a fresh device shows the account's reminders
  // without a page refresh (the desktop shell never hard-reloads).
  useEffect(() => {
    const reload = () => {
